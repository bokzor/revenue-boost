...
function FrequencyGroupSettings({
    ...
    const [maxPerSession, setMaxPerSession] = useState(
        settings?.max_per_session?.toString() ?? ""
    );
    const [maxPerDay, setMaxPerDay] = useState(
        settings?.max_per_day?.toString() ?? ""
    );
    const [cooldown, setCooldown] = useState(
        settings?.cooldown_between_popups?.toString() ?? ""
    );
    ...
    const handleChange = useCallback(
        (
            ...
            const newMaxPerSession =
                updates.max_per_session !== undefined
                    ? updates.max_per_session
                    : maxPerSession;
            const newMaxPerDay =
                updates.max_per_day !== undefined ? updates.max_per_day : maxPerDay;
            const newCooldown =
                updates.cooldown_between_popups !== undefined
                    ? updates.cooldown_between_popups
                    : cooldown;

            if (updates.enabled !== undefined) setEnabled(updates.enabled);
            if (updates.max_per_session !== undefined)
                setMaxPerSession(updates.max_per_session);
            ...
            onChange({
                enabled: newEnabled,
                max_per_session: newMaxPerSession
                    ? parseInt(newMaxPerSession, 10)
                    : undefined,
                max_per_day: newMaxPerDay ? parseInt(newMaxPerDay, 10) : undefined,
                cooldown_between_popups: newCooldown
                    ? parseInt(newCooldown, 10)
                    : undefined,
            });
        },
            ...
            {enabled && (
                <InlineGrid columns={3} gap="400">
                    ...
                    <TextField
                        ...
                        value={cooldown}
                        onChange={(value) =>
                            handleChange({ cooldown_between_popups: value })
                        }
                        autoComplete="off"
...
export function GlobalCappingSettings({
    settings,
    onChange,
}: GlobalCappingSettingsProps) {
            ...
            <BlockStack gap="400">
                <BlockStack gap="200">
                    ...
                    <Text as="p" tone="subdued">
                        Set limits on how often campaigns are shown to visitors across your
                        entire store. These settings override individual campaign rules when
                        "Respect global limits" is enabled.
                    </Text>
                </BlockStack>
                        ...
                        {selectedTab === 0 && (
                            <FrequencyGroupSettings
                                ...
                                onChange={(newSettings) =>
                                    onChange({ frequencyCapping: newSettings })
                                }
                            />
                        )}
                        {selectedTab === 1 && (
                            <FrequencyGroupSettings
                                ...
                                onChange={(newSettings) =>
                                    onChange({ socialProofFrequencyCapping: newSettings })
                                }
                            />
                        )}
                        {selectedTab === 2 && (
                            <FrequencyGroupSettings
                                ...
                                onChange={(newSettings) =>
                                    onChange({ bannerFrequencyCapping: newSettings })
                                }
                            />
                        )}
                    </Box>
