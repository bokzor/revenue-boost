...
export function varsFromDesign(design: PopupDesignConfig, size: PopupSize | undefined): { style: React.CSSProperties; 'data-size': string } {
  ...
  const style: React.CSSProperties = {
    ...
    // Dimensions & Shape
    '--rb-popup-radius': typeof design.borderRadius === 'number' ? `${design.borderRadius}px` : (design.borderRadius || '12px'),
    '--rb-popup-font-family': design.fontFamily || 'system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif',
    
    // Max widths based on size (matches spec)
    // small: ~400px, medium: ~560-600px, large: ~720-800px
    '--rb-popup-max-w': actualSize === 'large' ? '800px' : actualSize === 'medium' ? '600px' : '400px',
    
    // Padding based on size
    '--rb-popup-padding': actualSize === 'large' ? '2rem' : actualSize === 'medium' ? '1.5rem' : '1.25rem',
