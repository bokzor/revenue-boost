 ...
 * TagBadge Component
 *
 * A small promotional badge/pill displayed above headlines.
 * Used for text like "Exclusive offers inside" with optional icon.
 *
 * Uses CSS variables for styling:
 * - --rb-popup-badge-bg: Background color
 * - --rb-popup-badge-fg: Text color
...
/**
 * Icon component for badge icons
 */
function BadgeIconComponent({ icon, size = 14 }: { icon: BadgeIcon; size?: number }) {
...
export function TagBadge({
  ...
  return (
    <div
      className={`rb-tag-badge ${className}`}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: "0.5rem",
        padding: "0.375rem 0.75rem",
        borderRadius: "var(--rb-popup-badge-radius, 999px)",
        backgroundColor: backgroundColor || "var(--rb-popup-badge-bg, #F0FDF4)",
        color: textColor || "var(--rb-popup-badge-fg, #166534)",
        fontSize: "0.75rem",
        fontWeight: 500,
        lineHeight: 1,
      }}
    >
      {icon && icon !== "none" && <BadgeIconComponent icon={icon} size={14} />}
      <span>{text}</span>
    </div>
  );
}
