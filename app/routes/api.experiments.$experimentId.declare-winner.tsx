...
export async function action({ request, params }: ActionFunctionArgs) {
    try {
        ...
        if (!winningVariantKey) {
            return data(
                { success: false, error: "Winning variant key is required" },
                { status: 400 }
            );
        }
        ...
        const winningCampaign = campaigns.find(
            (c) => c.variantKey === winningVariantKey
        );

        if (!winningCampaign) {
            return data(
                { success: false, error: "Winning variant not found" },
                { status: 404 }
            );
        }
            ...
            prisma.experiment.update({
                ...
                data: {
                    trafficAllocation: {
                        ...
                        ...Object.fromEntries(
                            losingCampaigns.map((c) => [c.variantKey || "UNKNOWN", 0])
                        ),
                    },
                    status: "COMPLETED",
                },
        ...
        console.log(`[Declare Winner] Set ${winningVariantKey} as winner for experiment ${experimentId}`);
