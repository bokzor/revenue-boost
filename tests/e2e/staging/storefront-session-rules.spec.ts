...
test.describe.serial("Session Rules & Frequency Capping", () => {
  ...
  test("max impressions per session enforces limit in browser", async ({ page }) => {
    ...
    if (isVisibleAfterReload) {
      console.log(
        "⚠️ Popup appeared again after reload - frequency cap may not persist across reloads"
      );
  ...
  test("multiple impressions allowed when configured", async ({ page }) => {
    ...
    await page.reload();
    await handlePasswordPage(page);
    await page.waitForTimeout(5000); // Extra wait for logs
    await expect(popup).toBeVisible({ timeout: 15000 });
  ...
  test("session persistence survives page navigation", async ({ page }) => {
    ...
    if (!isVisibleOnCollections) {
      ...
      console.log(
        "⚠️ Collections page: Popup visible - frequency cap may not persist across navigation"
      );
    }
    ...
    if (!isVisibleOnHomeAgain) {
      ...
      console.log(
        "⚠️ Home page again: Popup visible - this is expected behavior in some configurations"
      );
    }
