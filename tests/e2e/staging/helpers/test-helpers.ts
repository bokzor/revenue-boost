 ...
 *
 * NOTE: There is NO API caching - verified that:
 * - Cloud Run has no caching
 * - Shopify App Proxy has no caching (cf-cache-status: DYNAMIC)
 * - Response has cache-control: private, no-store
 *
 * This delay exists to ensure the database write is fully committed
 * before the page loads and calls the API. 2.5 seconds should be sufficient.
 */
 ...
 */
export async function navigateToStorefrontFresh(
  page: Page,
...
export async function submitForm(page: Page) {
  const submitButton = page.locator(
    'button[type="submit"], button:has-text("Submit"), button:has-text("Subscribe")'
  );
...
export async function waitForPopupRendered(page: Page, timeout: number = 15000): Promise<boolean> {
  try {
    ...
    await page.locator("#revenue-boost-popup-shadow-host").waitFor({ state: "visible", timeout });
...
export async function isPopupVisible(page: Page): Promise<boolean> {
  return page
    .locator("#revenue-boost-popup-shadow-host")
    .isVisible()
    .catch(() => false);
}
...
export async function clickSpinButton(page: Page): Promise<boolean> {
  return page.evaluate(() => {
    ...
    const spinBtn = shadow.querySelector(
      'button[class*="spin" i], ' + 'button:has-text("Spin"), ' + '[class*="spin-button" i]'
    ) as HTMLButtonElement;
 ...
 */
export async function getTextFromShadowDOM(
  page: Page,
 ...
 */
export async function getFormInputsFromShadowDOM(
  page: Page
): Promise<{ email: boolean; checkbox: boolean; button: boolean }> {
  return page.evaluate(() => {
    const host = document.querySelector("#revenue-boost-popup-shadow-host");
    const shadow = host?.shadowRoot;
...
/**
 * Clean up ALL E2E test campaigns to avoid priority conflicts between test files.
 * This should be used in beforeEach to ensure a clean slate.
 */
export async function cleanupAllE2ECampaigns(prisma: any): Promise<number> {
  const deleted = await prisma.campaign.deleteMany({
    where: {
      OR: [
        { name: { startsWith: 'E2E-' } },
  ...
  if (deleted.count > 0) {
    console.log(`[Cleanup] Deleted ${deleted.count} E2E test campaigns`);
  }
...
/**
 * Maximum priority value for test campaigns.
 * Using 99999 ensures test campaigns always win over any other campaigns.
 */
export const MAX_TEST_PRIORITY = 99999;
...
export async function retryOperation<T>(
  ...
  const { maxRetries = 3, initialDelay = 500, maxDelay = 5000, onRetry } = options;
 ...
 */
export async function verifyNewsletterContent(
  page: Page,
  expected: {
    headline?: string;
  ...
  return page.evaluate((exp) => {
    ...
    if (emailInput && exp.emailPlaceholder) {
      ...
      if (!placeholder.includes(exp.emailPlaceholder.toLowerCase())) {
        errors.push(
          `Email placeholder "${exp.emailPlaceholder}" not found, got "${emailInput.placeholder}"`
        );
      }
    }
 ...
 */
export async function verifySpinToWinContent(
  page: Page,
  expected: {
    headline?: string;
    segments?: string[];
 ...
 */
export async function verifyFlashSaleContent(
  page: Page,
  expected: {
    headline?: string;
  ...
  return page.evaluate((exp) => {
    ...
    if (exp.hasCountdown) {
      ...
      const hasTimer =
        shadow.querySelector('[class*="countdown" i], [class*="timer" i], [data-countdown]') ||
 ...
 */
export async function verifyScratchCardContent(
  page: Page,
  expected: {
    headline?: string;
    hasCanvas?: boolean;
 ...
 */
export async function verifyExitIntentContent(
  page: Page,
  expected: {
    headline?: string;
    hasEmailInput?: boolean;
 ...
 */
export async function verifyFreeShippingContent(
  page: Page,
  expected: {
    threshold?: number;
    currency?: string;
 ...
 */
export async function verifySocialProofContent(
  page: Page,
  expected: {
 ...
 */
export async function verifyAnnouncementContent(
  page: Page,
  expected: {
    headline?: string;
 ...
 */
export async function verifyCountdownTimerContent(
  page: Page,
  expected: {
    headline?: string;
    hasTimer?: boolean;
 ...
 */
export async function verifyCartAbandonmentContent(
  page: Page,
  expected: {
    headline?: string;
  ...
  return page.evaluate((exp) => {
    ...
    if (exp.hasUrgencyTimer) {
      const hasTimer =
        shadow.querySelector('[class*="timer" i], [class*="countdown" i]') ||
        html.match(/\d{1,2}:\d{2}/);
      if (!hasTimer) {
 ...
 */
export async function verifyProductUpsellContent(
  page: Page,
  expected: {
    headline?: string;
    productCount?: number;
...
export async function getPopupState(page: Page): Promise<{
  ...
  return page.evaluate(() => {
    ...
    const html = shadow.innerHTML.toLowerCase();
    const isVisible =
      host instanceof HTMLElement && window.getComputedStyle(host).display !== "none";
 ...
 */
export async function verifyDiscountCodeDisplayed(
  page: Page,
  expectedCode?: string
): Promise<{
  found: boolean;
